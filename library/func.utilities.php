	...
	if ($StringDate == "") {
		...
		switch ($Mode) {
			...
			case 7:
				...
				break;
			/* 2004-11-28T09:13:55Z */
			case 8:
				return $aYear."-".$aMonth."-".$aDate."T".$aHour.":".$aMinute.":".$aSecond."Z";
				break;
			/* 23/03 */
			case 9:
				return $aDate."/".$aMonth;
...
http://www.zend.com/codex.php?id=947&single=1
*/
function ConvertMoneyFormatIndo2($string) {
...
example :
	Draw_DropDownList("SecondOfBirth", $SecondOfBirth, "selectsytle");
-----------------------------------------------------------------------------------*/
function DrawCategoryInsert($FormName, $FormValue = "", $FormStyle) {
...
format :
	Get_CategoryName($variable);

example :
	Get_CategoryName($iCategoryID);
-----------------------------------------------------------------------------------*/
function GetCategoryName($iCategoryID) {
	global $Tb_LUCategory;

	$SQL	= mysql_query("SELECT CategoryName FROM $Tb_LUCategory WHERE CategoryID = ".$iCategoryID."") or die(mysql_errno()." : ".mysql_error());

	while ($Query = mysql_fetch_object($SQL)) {
		$sCategoryName		= $Query->CategoryName;
