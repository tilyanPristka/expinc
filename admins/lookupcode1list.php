...
if($sAct=="del" && !empty($iLookUpAcctID)):
	//delete
	mysql_query("DELETE FROM tilyan_".$sFolder."_lookup_code1 WHERE ID = '".$iLookUpAcctID."'") or die(mysql_error());
	mysql_query("optimize table tilyan_".$sFolder."_lookup_code1") or die(mysql_errno()." : ".mysql_error());
...
<body>
	<div id="main">
        ...
        <div class="basic" style="float:left; margin-left: 2em;" id="mylist">
            	...
            	<table width="95%" cellpadding="2" cellspacing="2" border="0" class="OSStyle">
					<tr>
						...
						<td valign="top">
                            ...
                            <p><button onclick="javascript:document.location.href = 'lookupcode1add.php?id=<?php echo $iCoID?>';">Add New Look Up</button>&nbsp;&nbsp;<button onclick="javascript:document.location.href = 'ClientEdit.php?id=<?php echo $iCoID?>';">Return To Client Edit</button></p>
						</td>
					</tr>
				</table>
            </div>
            <a>Client's Look Up Code I</a>
            <div style="background-color:#FFF;">
			<?php
			/* Paginate */
			$Pg			= $_GET["pg"];
			...
			$sPaging		 = "SELECT ID FROM tilyan_".$sFolder."_lookup_code1";
			...
			?>
			<?php
			$sList		 = "SELECT * FROM tilyan_".$sFolder."_lookup_code1 ORDER BY CodeName LIMIT $Pg, $Limit";
				...
				<?php
					...
					$bgcolor = "#336699";
				  	...
				  	while($oList = mysql_fetch_object($SQLList)) {
						...
						$aDateDifference	= dateDifference($oList->StartDate, $oList->EndDate);
				?>
				<tbody>
				<tr>
					<td style="text-align:center; border-bottom:1px solid #990000;"><?php echo $oList->CodeName?></td>
					...
					<td style="text-align:center; border-bottom:1px solid #990000;"><a href="lookupcode1edit.php?lid=<?php echo $oList->ID?>&id=<?php echo $iCoID?>" class="normalbiglink" title="click to edit">edit</a> | <a href="javascript:Delete(<?php echo $iCoID?>,<?php echo $oList->ID?>)" class="normalbiglink" title="delete this from listing">delete</a></td>
				</tr>
				</tbody>
				<?php
