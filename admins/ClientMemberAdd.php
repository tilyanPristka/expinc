...
if ($_POST['bSubmitted']) {
	...
	if (!$bError) {
		if (empty($_GET['cpid'])) {
			/* INSERT */
			mysql_query("INSERT INTO tilyan_membersperson (CoID, CP_DateCreated, CP_User, CP_Pass, CP_Fullname, CP_Email, CP_NickName, CP_Permission, CP_Enabled) VALUES (".$iCoID.", now(), '".$CP_User."', '".$CP_Pass."', '".$CP_Fullname."', '".$CP_Email."', '".$CP_NickName."', '".$CP_Permission."', '".$CP_Enabled."')") or die(mysql_errno()." : ".mysql_error());
		} else {
			/* UPDATE */
			mysql_query("UPDATE tilyan_memberperson SET CP_User = '".$CP_User."', CP_Pass = '".$CP_Pass."', CP_NickName = '".$CP_NickName."', CP_Fullname = '".$CP_Fullname."', CP_Email = '".$CP_Email."', CP_Enabled = '".$CP_Enabled."' WHERE CP_ID = '".$_GET['cpid']."'") or die(mysql_errno()." : ".mysql_error());
		}
        ...
        <div class="basic" style="float:left; margin-left: 2em;" id="mylist">
			...
			<table width="95%" cellpadding="2" cellspacing="3" border="0" align="center" class="OSStyle">
				...
				<tr>
					...
					<td><input type="text" name="CP_Pass" id="CP_Pass" size="35" value="<?php echo $CP_Pass?>" /></td>
				</tr>
				<tr>
				...
				<tr>
					<td></td>
					<td><input type="submit" name="bnSubmit" id="bnSubmit" value="<?php echo (!empty($_GET['cpid']) ? "Update" : "Save")?>">&nbsp;&nbsp;<input type="button" name="bnCancel" id="bnCancel" value="Cancel" onClick="Cancel('clientmember.php?id=<?php echo $iCoID?>');"><?php if (!empty($_GET['cpid'])) { ?>&nbsp;&nbsp;<input type="button" name="bnDelete" id="bnDelete" value="Delete" onClick="Delete(<?php echo $_GET['cpid']?>)"><?php } ?></td>
				</tr>
			</table>
			</form>
