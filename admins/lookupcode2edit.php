...
if ($_POST['bSubmitted']) {
	...
	if (!$bError) {
		/* UPDATE */
		mysql_query("UPDATE tilyan_".$sFolder."_lookup_code2 SET CodeName = '".$CodeName."', CodeDesc = '".$CodeDesc."', ContractNo = '".$ContractNo."', ContractValue = '".$ContractValue."', StartDate = '".$StartDate."', EndDate = '".$EndDate."', Status = '".$Status."' WHERE ID = '".$_GET['lid']."'") or die(mysql_errno()." : ".mysql_error());

		echo "<meta http-equiv=\"refresh\" content=\"0;URL=lookupcode2list.php?id=".$iCoID."\">";
		exit();

	}
} else {
	if (!empty($_GET['lid'])):
		$SQLEditLookUp	= mysql_query("SELECT * FROM tilyan_".$sFolder."_lookup_code2 WHERE ID = '".$_GET['lid']."'") or die(mysql_errno()." : ".mysql_error());
	
		if ($oEditLookUp = mysql_fetch_object($SQLEditLookUp)):
			$CodeName		= stripslashes($oEditLookUp->CodeName);
			$CodeDesc		= stripslashes($oEditLookUp->CodeDesc);
			$ContractNo		= stripslashes($oEditLookUp->ContractNo);
			$ContractValue	= stripslashes($oEditLookUp->ContractValue);
			$StartDate		= stripslashes($oEditLookUp->StartDate);
			$EndDate		= stripslashes($oEditLookUp->EndDate);
			$Status			= stripslashes($oEditLookUp->Status);

			if(!empty($StartDate)):
				$aStartDate			= explode("-", $StartDate);
				$StartDateTanggal	= $aStartDate[2];
				$StartDateBulan		= $aStartDate[1];
				$StartDateTahun		= $aStartDate[0];
			else:
				$StartDateTanggal	= "";
				$StartDateBulan		= "";
				$StartDateTahun		= "";
			...
			if(!empty($EndDate)):
				...
				$EndDateTanggal	= $aEndDate[2];
				$EndDateBulan	= $aEndDate[1];
				$EndDateTahun	= $aEndDate[0];
			else:
				$EndDateTanggal	= "";
				$EndDateBulan	= "";
				$EndDateTahun	= "";
...
function Delete(iID,iLID) {
	if (confirm('You are about to delete one of the available Look Up.\nAre you sure ?')) { document.location.href = 'lookupcode2list.php?id='+ iID +'&lid='+ iLID +'&act=del'; }
}
//-->
</script>
</head>
...
<body>
	<div id="main">
        ...
        <div class="basic" style="float:left; margin-left: 2em;" id="mylist">
			...
			<a>Client's Look Up Code II Edit</a>
			<div style="background-color:#FFF; margin-bottom:15px; margin-top:15px;">
			...
			<table width="95%" cellpadding="2" cellspacing="3" border="0" align="center" class="OSStyle">
				...
				<tr>
					<td></td>
					<td><input type="submit" name="bnSubmit" id="bnSubmit" value="<?php echo (!empty($_GET['lid']) ? "Update" : "Save")?>">&nbsp;&nbsp;<input type="button" name="bnCancel" id="bnCancel" value="Cancel" onClick="Cancel('lookupcode2list.php?id=<?php echo $_GET["id"]?>');"><?php if (!empty($_GET['lid'])) { ?>&nbsp;&nbsp;<input type="button" name="bnDelete" id="bnDelete" value="Delete" onClick="Delete(<?php echo $_GET['id']?>,<?php echo $_GET['lid']?>)"><?php } ?></td>
				</tr>
			</table>
			</form>
