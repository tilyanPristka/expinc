...
if ($_POST['bSubmitted']) {
	...
	if (!$bError) {
		/* UPDATE */
		mysql_query("UPDATE tilyan_".$sFolder."_lookup_code1 SET CodeName = '".$CodeName."', CodeDesc = '".$CodeDesc."', ContractNo = '".$ContractNo."', ContractValue = '".$ContractValue."', StartDate = '".$StartDate."', EndDate = '".$EndDate."', Status = '".$Status."' WHERE ID = '".$_GET['lid']."'") or die(mysql_errno()." : ".mysql_error());
	...
	if (!empty($_GET['lid'])):
		$SQLEditLookUp	= mysql_query("SELECT * FROM tilyan_".$sFolder."_lookup_code1 WHERE ID = '".$_GET['lid']."'") or die(mysql_errno()." : ".mysql_error());
...
function Delete(iID,iLID) {
	if (confirm('You are about to delete one of the available Look Up.\nAre you sure ?')) { document.location.href = 'lookupcode1list.php?id='+ iID +'&lid='+ iLID +'&act=del'; }
}
//-->
</script>
</head>
...
<body>
	<div id="main">
        ...
        <div class="basic" style="float:left; margin-left: 2em;" id="mylist">
			...
			<a>Client's Look Up Code I Edit</a>
			<div style="background-color:#FFF; margin-bottom:15px; margin-top:15px;">
			...
			<table width="95%" cellpadding="2" cellspacing="3" border="0" align="center" class="OSStyle">
				...
				<tr>
					<td></td>
					<td><input type="submit" name="bnSubmit" id="bnSubmit" value="<?php echo (!empty($_GET['lid']) ? "Update" : "Save")?>">&nbsp;&nbsp;<input type="button" name="bnCancel" id="bnCancel" value="Cancel" onClick="Cancel('lookupcode1list.php?id=<?php echo $_GET["id"]?>');"><?php if (!empty($_GET['lid'])) { ?>&nbsp;&nbsp;<input type="button" name="bnDelete" id="bnDelete" value="Delete" onClick="Delete(<?php echo $_GET['id']?>,<?php echo $_GET['lid']?>)"><?php } ?></td>
				</tr>
			</table>
			</form>
