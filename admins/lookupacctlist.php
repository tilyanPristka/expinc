...
if($sAct=="del" && !empty($iLookUpAcctID)):
	//delete
	mysql_query("DELETE FROM tilyan_".$sFolder."_lookup_account WHERE ID = '".$iLookUpAcctID."'") or die(mysql_error());
	mysql_query("optimize table tilyan_".$sFolder."_lookup_account") or die(mysql_errno()." : ".mysql_error());
...
<body>
	<div id="main">
        ...
        <div class="basic" style="float:left; margin-left: 2em;" id="mylist">
            	...
            	<table width="95%" cellpadding="2" cellspacing="2" border="0" class="OSStyle">
					<tr>
						...
						<td valign="top">
                            ...
                            <p><button onclick="javascript:document.location.href = 'lookupacctadd.php?id=<?php echo $iCoID?>';">Add New Look Up</button>&nbsp;&nbsp;<button onclick="javascript:document.location.href = 'ClientEdit.php?id=<?php echo $iCoID?>';">Return To Client Edit</button></p>
						</td>
					</tr>
				</table>
            </div>
            <a>Client's Look Up Acct. #</a>
            <div style="background-color:#FFF;">
			<?php
			/* Paginate */
			$Pg			= $_GET["pg"];
			...
			$sPaging		 = "SELECT ID FROM tilyan_".$sFolder."_lookup_account";
			...
			for ($k = 1;$k <= $Hal;$k++) {
				$newpg = $Limit*($k-1);
				if($Pg != $newpg) {
					print " <a href=\"?id=".$iCoID."&pg=".$newpg."\" title=\"page ".$k."\" class=\"normallink\">".$k."</a> \n";
				} else {
			...
			?>
			<?php
			$sList		 = "SELECT * FROM tilyan_".$sFolder."_lookup_account ORDER BY AccNo LIMIT $Pg, $Limit";
			...
			<table width="100%" align="center" cellpadding="2" cellspacing="2" border="0">
				<thead>
				<tr>
					<th style="background-color:#b7cae8; color:#000000; text-align:center;">Acct. #</th>
					<th style="background-color:#b7cae8; color:#000000; text-align:center;">Account Name</th>
					<th style="background-color:#b7cae8; color:#000000; text-align:center;">Type</th>
				...
				<tbody>
				<tr>
					<td style="bgtext-align:center; border-bottom:1px solid #990000;"><?php echo $oList->AccNo?></td>
					<td style="border-bottom:1px solid #990000;"><?php echo nl2br($oList->AccName)?></td>
					<td style="text-align:center; border-bottom:1px solid #990000;"><?php echo $oList->Type?></td>
					<td style="text-align:center; border-bottom:1px solid #990000;"><?php echo $sStatusWords?></td>
					<td style="text-align:center; border-bottom:1px solid #990000;"><a href="lookupacctedit.php?lid=<?php echo $oList->ID?>&id=<?php echo $iCoID?>" class="normalbiglink" title="click to edit">edit</a> | <a href="javascript:Delete(<?php echo $iCoID?>,<?php echo $oList->ID?>)" class="normalbiglink" title="delete this from listing">delete</a></td>
				</tr>
				</tbody>
				<?php
