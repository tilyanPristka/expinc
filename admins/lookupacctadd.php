...
if ($_POST['bSubmitted']) {
	...
	if (!$bError) {
		/* INSERT */
		mysql_query("INSERT INTO tilyan_".$sFolder."_lookup_account (AccNo, AccName, Type, Status) VALUES ('".$AccNo."', '".$AccName."', '".$Type."', '".$Status."')") or die(mysql_errno()." : ".mysql_error());
...
<body>
	<div id="main">
        ...
        <div class="basic" style="float:left; margin-left: 2em;" id="mylist">
			...
			<a>Client's Look Up Account Add</a>
			<div style="background-color:#FFF; margin-bottom:15px; margin-top:15px;">
			...
			<table width="95%" cellpadding="2" cellspacing="3" border="0" align="center" class="OSStyle">
				...
				<tr>
					<td></td>
					<td><input type="submit" name="bnSubmit" id="bnSubmit" value="<?php echo (!empty($_GET['lid']) ? "Update" : "Save")?>">&nbsp;&nbsp;<input type="button" name="bnCancel" id="bnCancel" value="Cancel" onClick="Cancel('lookupacctlist.php?id=<?php echo $iCoID?>');"><?php if (!empty($_GET['lid'])) { ?>&nbsp;&nbsp;<input type="button" name="bnDelete" id="bnDelete" value="Delete" onClick="Delete(<?php echo $_GET['lid']?>)"><?php } ?></td>
				</tr>
			</table>
			</form>
