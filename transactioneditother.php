...
if ($_POST['bSubmitted']):
	...
	for ($iLoopRec = 1; $iLoopRec <= $iRecNo; $iLoopRec++) {
		...
		$bDRCR[$iLoopRec]		= TrimString($_POST['bDRCR'.$iLoopRec.'']);

		if($bDRCR[$iLoopRec]=="DR"):
			$iTotalDR = $iTotalDR+$iAmount[$iLoopRec]; 
		elseif($bDRCR[$iLoopRec]=="CR"):
			$iTotalCR = $iTotalCR+$iAmount[$iLoopRec];
		endif;
	...
	$iDifferenceTotal = $iTotalDR-$iTotalCR;

	if($iChoice=="1"):
		$iRecNo = $iRecNo+1;
	elseif($iChoice=="2"):
		if($iDifferenceTotal==0):
			for ($iLoopRec = 1; $iLoopRec <= $iRecNo; $iLoopRec++) {
				if(!empty($sDesc[$iLoopRec])):
					if(!empty($iTransID[$iLoopRec])):
						/* UPDATE */
						mysql_query("UPDATE tilyan_".$_SESSION['TLY__MemberFolder']."_account SET DocID = ".$iDocID.",AccNo = '".$sAccNo[$iLoopRec]."',AccName = '".$sAccName[$iLoopRec]."',Description = '".$sDesc[$iLoopRec]."',Amount = ".$iAmount[$iLoopRec].",DRCR = '".$bDRCR[$iLoopRec]."' WHERE TransID = ".$iTransID[$iLoopRec]."") or die(mysql_errno()." : ".mysql_error());
						//echo "UPDATE tilyan_".$_SESSION['TLY__MemberFolder']."_account SET DocID = ".$iDocID.",AccNo = '".$sAccNo[$iLoopRec]."',AccName = '".$sAccName[$iLoopRec]."',Description = '".$sDesc[$iLoopRec]."',Amount = ".$iAmount[$iLoopRec].",DRCR = '".$bDRCR[$iLoopRec]."' WHERE TransID = ".$iTransID[$iLoopRec]."<br /><br />";
					else:
						/* INSERT */
						//mysql_query("INSERT INTO tilyan_".$_SESSION['TLY__MemberFolder']."_account (DocID,AccNo,AccName,Description,Amount,DRCR) VALUES (".$iDocID.", '".$sAccNo[$iLoopRec]."', '".$sAccName[$iLoopRec]."', '".$sDesc[$iLoopRec]."', ".$iAmount[$iLoopRec].", '".$bDRCR[$iLoopRec]."')") or die(mysql_errno()." : ".mysql_error());
			...
			$sAlertDifference = "The DR and CR entry for this document is not balance. There is a difference of ".ConvertMoneyFormatIndo2($iDifferenceTotal)." in the calculation.";
		endif;
	endif;
else:
        ...
        <div class="other" style="margin-bottom:6px;">
			...
			<table width="95%" cellpadding="2" cellspacing="3" border="0" align="center" class="OSStyle">
                ...
                <tr>
					...
					<td colspan="5" style="font-size:12px; font-weight:bold;"><?php echo $sRefDocFullNo; ?></td>
				</tr>
                <?php
				endif;
				?>
				<tr>
					...
					<td colspan="5"><strong><?php echo $sDocFullNo ?></strong></td>
				</tr>
				<tr>
				...
				<tr>
					<td style="border-bottom: solid 1px #990000;" colspan="6">&nbsp;</td>
				</tr>
                <?php
					if($sAlertDifference!=""):
				?>
				<tr>
					<td style="border-bottom: solid 1px #990000; padding:6px 0 6px 0; background-color:#990000; font-weight:bold; font-size:12px; text-align:center; color:#FFF;" colspan="6"><?php echo $sAlertDifference; ?></td>
				</tr>
                <?php
					endif;
				?>
				<tr>
					...
					<td style="background-color:#CCCCCC; height:22px; padding-left:6px;"><strong>DR / CR</strong></td>
				...
				<tr>
					...
					<td align="center">
                        <select name="bDRCR<?php echo $iRecLoop?>" id="bDRCR<?php echo $iRecLoop?>" class="SELECT">
                            <option value="DR"<?php if($bDRCR[$iRecLoop]=="DR") { echo " selected"; }; ?>>DR</option>
                            <option value="CR"<?php if($bDRCR[$iRecLoop]=="CR") { echo " selected"; }; ?>>CR</option>
                        </select>
					</td>
